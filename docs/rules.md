...
qooMeta の規則は、次の 2 つの JSON に書かれています(形式の版は filename-formats.json が第 6 版、series-rules.json が第 2 版)。どちらもパッケージに同梱された**既定値**です。
...
| `"$order": ["ID", …]` | 巻の読み手と語の規則(`markers`) | 挙げた ID を、この順で先頭に寄せる(挙げなかったものは既定の順で後ろに続く) |
...
- **例外: 同梱の 2 つの規則ファイル(filename-formats.json・series-rules.json)は、2026-09-21 に利用者の手元の設定を
  取り込んだので、利用者の蔵書に由来する語を含みます**(ルールセットの `auto`・`plain` の語、巻の単位の語など)。
  この 2 つは禁止語の検査から外してあります(利用者の判断)。**そこにある語を、docs・コード・テスト・コミットメッセージへ
  写さないでください。** 説明には架空の語を使い、既定の語は「同梱の一覧を参照」とします。
...
| `schemaVersion` | 形式の版。filename-formats.json は `6`、series-rules.json・examples.json・rules-bundle は `2` |
...
規則・パラメータ・一覧には `"since": 番号` を書けます(その番号の水準の qooMeta から働く)。今の本体の水準(`engineLevel`)は `10` です
(番号ごとに何を足したかは [rules-format-design.md](rules-format-design.md)「版の番号」)。

---

## 1. filename-formats.json(第 6 版)
...
| `separators` | 著者の値を分ける文字列(省いたときは `,` `，` `、`)。並びの欄は著者だけ |
| `defaults` | 名前に書かれていない欄(ジャンル・イベント・原作・情報)に入れる値 |
| `plain` | **型として読まない文字列**(`words` と `patterns`)。名前の中のこの部分は、括弧でも型の括弧として読まず、値にはそのまま残します。同梱のどのプリセットにも丸括弧の中の西暦の正規表現が入っています(`月の庭 (2026)` の `(2026)` を原作や巻数にしない)。語もプリセットごとにいくつか入っています(同梱の一覧を参照)。プリセットと型に書けて、足し合わさります |
| `ignoreBracketsInsideTitle` | 題の**途中**(頭でも末尾でもない所)の括弧を、読み残しに数えないか(プリセットごと。省くと `true`)。型が括弧を欄として読むのは名前の頭・著者の直後・末尾だけなので、途中の括弧は型を直しても欄にならず、数えると直しようのない警告になる |
| `auto` | 本ごとにこのプリセットを自動で選ぶ条件(`words`・`headRequired`・`headExcluded`)。下の「本ごとにプリセットを選ぶ」 |

**ファイル全体の段はありません**(第 6 版。2026-09-21、利用者の指示。設定はルールセットごと)。
**`separators` と `defaults` は、プリセットと型の 2 か所に書けて、内側に書いたものが勝ちます**
(型 > プリセット)。`separators` は書いた所で丸ごと置き換わり(足し合わせません)、`defaults` は欄ごとです。
...
**本ごとにプリセットを選べます**(2026-09-21、利用者の指示)。

- アプリの段 2 では、プリセットを 1 つ選ぶか「自動」を選びます。「自動」は、プリセットの `auto` の条件で本ごとに選ぶもので、
  **全冊が決まるときだけ**選べます(決まらない本を、黙ってどれかで読まないため)。
- 段 3 の一覧では、行の右クリック(「ファイル名を再解析」)で、選んだ本だけを別のプリセットで読み直せます。
- 決まった割り当ては、作業ファイルの `presets.folders` に本の ID ごとに入ります。条件の書き方と決まり方は
  [filename-format.md](filename-format.md) の 4「本ごとにルールセットを選ぶ」。
- CLI では `--presets <割り当て.json>` で割り当てを渡せます:
  `{ "default": "commercial", "folders": { "相対パス": "doujinshi" } }`(相対パスの頭で当て、長い頭が勝つ。本の ID をそのまま書けば
  その本だけ)。**蔵書のフォルダ名を含むので、リポジトリの外に置きます。**
...
並べ替えられます**(上から試し、先に当たった規則が勝つ)。オブジェクトで書いてある所は、処理の順をコードが決めていて、並べ替えられません(`grouping` の処理の順は下の 4)。

1. **比べる単位**: 同じ書き手(著者の並びの先頭。無ければいちばん内側のフォルダ)で、本の種別(`@genre`)が同じ本どうしだけを比べます(方針 `differentGenre`)。
2. **`compare`**: 比べるための形(全角・半角、大文字・小文字、飾りの記号、異体字をそろえる)。
3. **`markers`**: タイトルの中の「役目のある語」(版・入手経路の印、総集編の語)を見つけます。**並び順が優先順位**です。
4. **`grouping`**: 組を作ります(処理の順は、総集編の本を分けておく → 1 段目 → 2 段目 → 総集編の組 → 組どうしをまとめる →
   ネタ違いで分ける → 版違いだけの組を捨てる)。1 段目 `volumeHead`(「タイトル + 巻」を頭でまとめる。副題付きの本と `attachAcrossScript` の本も
   ここで入れる)、2 段目 `sharedPrefix`(先頭の共通部分)、総集編 `compilation`、組どうしをまとめる `mergeVolumeSubgroups`・
   `mergeSubseries`、組にしない例外(`conditions`、ネタ違い、版違いだけ)。
...
| `keepFollowing` | 文字 | 共通部分のすぐ後ろにあれば、シリーズ名に含める文字(`月の庭！` の `！`、`ご褒美♡2` の `♡`。同梱は `!` `?` `！` `？` `♡` `♥`)。巻の前では読み飛ばすので、`ご褒美♡2` は巻 2 |
| `brackets` | 対応表 | 閉じ括弧 → 開き括弧 |
| `labelIntroducers` | 語 | シリーズ名の最後の 1 語がこの語なら外す(`X side A` と `X side B` のシリーズ名は `X`) |
| `editionWords` / `sourceWords` | 語 | 版の印 / 入手経路の印(同梱の `sourceWords` は空。`DL版` は `markers.source` の正規表現) |
...
| `particles` | 語 | 助詞とみなすひらがな(既定は `の` `と` `は` `に`)。シリーズ名に区切りなしで続く残りがこれで始まるなら、巻数(表示)にしない(規則 `volume.particles`。`since` 10) |
| `plainWords` | 語 | そのまま読む語(語の規則 `plain`。同梱の一覧を参照) |
| `standaloneWords` | 語 | シリーズに入れない語(語の規則 `standalone`。同梱は空) |
...
| `kanjiAloneDigits` | 語 | それだけで巻になる漢数字(巻の読み手 `kanjiAlone` の `digits`)。同梱は大字の `壱`〜`玖`(七は `漆` `柒` の両方)、旧字体の `壹` `貳` `參`、ふつうの漢数字の `一`〜`九` の 22 字(2026-09-21、利用者の指示。前はふつうの漢数字を入れていなかった)。`X 弐` は 2 巻、`X 四` は 4 巻 |
...
見分けた結果を**どう扱うか**。正解が 1 つあるわけではないので、好みで選びます。
...
方針の値が JSON から欠けているときだけ、各方針の選べる値の最初のもの(`RuleSchema.policies`)が使われます。これは取り込む前の
既定なので、`compilations` と `differentRelation` では同梱の値と違います。
...
- `unnumberedVolume` の `asWritten`(既定): シリーズ名より後ろが巻として読めなくても、その文字列(「アフターエピソード」
  「夏美の場合」)をそのまま巻数(表示)に入れます。**並べ替えの数は付けません** ―― 名前から順番は決められないので、
  分かる順番だけを出し、残りは空のまま利用者に渡します(2026-09-22、利用者の判断)。語の途中で切れた残り(「の安息」)は
  入れません(総集編の語で始まる残りは、区切りが無くても入れます)。順番を付けたい本は、一覧で巻数(表示)を直すか、
  連番を振れば数が入ります。**1 巻の推定のあとに働く**ので、番号があって 1 が空いているシリーズの副題付き 1 冊は、
  これまでどおり 1 巻になります。
- 総集編の語が**題名の途中にあるだけ**で、その名前が指す本編が無い本(`X・食／総集編`)は、ふつうの本として
  名前の頭が合う組へ入れます(語が 1 つ入っているせいでシリーズから外れないように)。
- `compilations` の `inMainSeries`: 同じ書き手に本編のシリーズ `X` があれば、総集編をそこへ入れます。**本編がシリーズになって
  ...
  既定と同じく `X 総集編` のシリーズにします。本編の組の名前が「`X` + ひらがなだけ」になっているとき(`月の庭の安息` と
  `月の庭の宴` だけで組を作ると、名前が `月の庭の` になる)は、総集編の言う名前 `X` に直してその組へ入れます(共通部分が
  助詞で終わるのは 2 段目の弱点で、総集編はシリーズ名がどこまでかを名前で言っているため)。`compilationVolume` の `afterRange` では、収録範囲が読めた総集編の巻を、その最後の巻の直後に
...
- 例のファイルでは、例ごとに `"policies": { … }` を書いて、その方針での結果を確かめられます(規則の値は `"settings"`。
  [rules-format-design.md](rules-format-design.md)「例のファイル」)。
...
| 規則(同梱の順) | `treat` | 意味 |
|---|---|---|
| `plain` | `keep` | **そのまま読む語**(`@list:plainWords`)。何もしない(下の規則と、後ろの段階の**巻の読み手**から語を守る。題名が「No.5」の本は、ここに足せば巻 5 になりません)。同梱は「フルカラー総集編」など: 独立した 1 冊で、版違いでも総集編でもない |
...
| `standalone` | `standalone` | **シリーズに入れない語**。この語のある本は、どのシリーズにも入れません(一覧で「シリーズに入れない」と直した本と同じ扱い)。**同梱の一覧 `standaloneWords` は空**で、使う人が足します(`since` 2) |
...
先にネタ・キーワードとして読まれるので、そこにある印は見分けられません。初回限定版・限定版・特装版・通常版は、どの版の本かを
言う語なので版の印に入れています(2026-09-20。前は入手経路に入れていた)。**同梱の一覧 `sourceWords` は空**で、入手経路の印は
`source` の正規表現(`DL版`)だけです。規則を止める(`"enabled": false`)と、その印を見分けません。

### `grouping` — 組の作り方と例外

| 規則 | パラメータ(既定) | 意味 |
|---|---|---|
| `compilation` | `singleWhenMainExists`(false) | 総集編(語は `markers` の `compilationMark`)を本編とは別の `X 総集編` のシリーズにする(方針 `compilations` が `ownSeries` のとき。`inMainSeries` でも、本編が見つからない総集編はこの形になる)。2 冊以上か、`singleWhenMainExists` が入で同じ書き手に本編のシリーズ `X` があれば 1 冊でもシリーズにする。同梱の既定は 2026-09-21 に false にした |
| `volumeHead` | — | 1 段目: 「タイトル + 巻」の形の本を、巻を除いた頭でまとめる。後ろが巻だけなので頭は 1 文字でもよい。副題付きの本(方針 `subtitled`)も、頭が語の切れ目で一致する組へここで入れる |
| `attachAcrossScript` | (入切だけ。既定は入。`since` 7) | 区切りなしで続く副題(`X リベンジ`)を、巻でまとまった組へ入れる。文字の種類が変わり、続きがひらがなでないときだけ(ひらがなが続くのは語の途中)。**新しい組を作るのには使わない**。同じ見方を、組どうしをまとめるとき(下の 2 つ)と、総集編の語が題名の途中にあるだけの本を組へ入れるときにも使う |
| `mergeVolumeSubgroups` | (入切だけ。既定は入。`since` 8) | 組の名前が「別の組の名前 + 巻」(`X 6巻`)になっているなら、その別の組へ入れる。巻として読めるときだけ。入れ先は名前がいちばん短い組 |
| `mergeSubseries` | (入切だけ。既定は入。`since` 9) | 組の名前が「別の組の名前 + 副題」(`X eve 甲` が `X eve 甲 3`・`X eve 甲 4` と自前の番号を持つ組)になっているなら、それも別の組へ入れる。**入れる側も入れ先も、巻でまとまった組(`volumeHead`)のときだけ**(題の頭が同じだけの 2 段目の組へ、番号の並んだシリーズを吸い込まない)。巻は副題ごと書いたもの(`eve 甲 3`)になり、本編の巻と重ならない。総集編の組は動かさない(方針 `compilations`)。切のときは、`X 外伝 1・2` のような組は別のシリーズに残る |
| `sharedPrefix` | `minPrefix`(4)、`minWholeTitle`(2) | 2 段目: 残りを先頭の共通部分でまとめる。共通部分が**語の途中で**切れるときは `minPrefix` 文字以上、片方のタイトル全体がもう片方の先頭と一致するとき(`XY` と `XY2`)は `minWholeTitle` 文字以上。タイトルを並べて隣どうしを比べるので、組より**前**に並んだために外れた 1 冊は、あとで拾う(下) |
| `sharedPrefix.conditions.reject-hiragana-ending` | — | 語の途中で切れる共通部分が、ひらがな(助詞など)で終わるなら組にしない。**片方のタイトル全体がもう片方の頭と一致する形(`minWholeTitle`)にも効く**: 一致がひらがなで終わり、長いほうがそのままひらがなで続くなら組にしない(`〜です!!` と `〜ですか?`。記号を無視して比べると `〜です` と `〜ですか` になる)。続きが漢字やカタカナなら組にする(`月の庭の` と `月の庭の安息`) |
...
- **2 段目の取りこぼしを拾う**(規則ではなく、いつも働く。2026-09-21、利用者の報告): 2 段目はタイトルを並べて隣どうしを
  比べるので、組より前に並んだ 1 冊が、隣の本との長い共通部分(助詞で終わる)で断られて外れることがあった
  (`X に甲…` が `X に乙…` に断られ、あとに来た `X 延長戦` だけが `X に乙…` と組になる)。1 冊だけ残った本の頭が、前後で
  いちばん近い 2 冊以上の組の共通部分と同じなら、その組へ入れます。
...
| `includeFollowing` | `characters`(`@list:keepFollowing`) | 共通部分のすぐ後ろの記号(`!` `?` `♡` …)をシリーズ名に含める。含めた記号は、巻を読むときには巻の前の区切りとして読み飛ばす(`ご褒美♡2` は、シリーズ名 `ご褒美♡`、巻 2) |
...
| `wordNumber`(`since` 6) | `numberWord` | `words`(`@list:numberWords`) | **数を語で書いた巻**(`に` = 2、`さん` = 3、`ふたつ` = 2、`みっかめ` = 3)。対応表に無い語は読まない。語がひとまとまりで書かれているときだけ(`ふたつの影` は読まない) |
| `kanjiAlone`(`since` 4) | `kanjiAloneNumeral` | `digits`(`@list:kanjiAloneDigits`) | **一覧の漢数字だけでできた巻**(`X 弐` = 2、`X 肆` = 4、旧字体の `壹` `貳` `參` も同じ。`X-壹-副題` のように区切りに挟まれた形も読む)。前に語が無くても後ろに単位が無くても読む。語の切れ目まで求めるので `参加者たち` は読まない。同梱の `digits` には、ふつうの漢数字の `一`〜`九` も入っている(`X 四` = 4)。十より大きい数は `第`・`巻` などが要る |
...
| `sequel`(`since` 3) | `sequel` | `words`(`@list:sequelWords`) | **既定では止めてある**(入にすると効く)。本編の後に続く 1 冊(`アフターエピソード` `後日談` `外伝` `番外編` `特別編` `おまけ` …)。英字は大文字小文字を問わない。**シリーズ名より後ろがこの語で始まるとき**、残り全体を巻の表記にし、並べる数はそのシリーズの**最後の番号 + 1**(`後日談 2` のように番号が続けば + その数)。巻の表記が総集編の語で始まる本は、本編のナンバリングではないので最後の番号には数えない。番号がどこにも無いシリーズでは、表記だけを入れて数は付けない |
...
(`since` 5。`enabled` を持たず、いつも働く。)数字を巻として読むのは、そのすぐ後ろが**単位・空白・名前の終わり**か、この一覧(`@list:volumeFollowers`)の文字のときだけです
(`月の庭 4ー夜編ー` の `ー`)。ここに無い文字が続く数字は、巻ではなく題名の一部として扱います。

#### `particles` — 助詞とみなすひらがな

(`since` 10。パラメータは `words`、既定は `@list:particles`。)シリーズに入った本で巻が読めないとき、シリーズ名より後ろの文字列をそのまま巻数(表示)にします(方針 `unnumberedVolume`)。
...
| `firstVolume` | `excludeMarkers`(`@list:notFirstMarkers`)、`excludePrefixes`(`@list:notFirstPrefixes`) | 番号の無い 1 冊を 1 巻とみなす(方針 `unnumberedFirst`)。この語がある本は候補にしない。候補が 2 冊以上なら、タイトルがシリーズ名だけの本を採る。版違い(印を除いて同じタイトル)は 1 冊と数える。ほかの巻がゼロ埋め(`02`)なら `01`、ローマ数字なら `I`、漢数字・大字なら `一`・`壱`、数の語や位置の語ならそろいの語、のように表記をほかの巻にそろえる |
