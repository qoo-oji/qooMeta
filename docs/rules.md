...
| キー | 意味 |
|---|---|
| `defaultPreset` | 本がプリセットを選ばなかったときに使う名前(同梱は `commercial`) |
| `presets` | 名前を付けた型の並び。同梱は `doujinshi`(同人誌。16 通り)・`doujinshi-event`(先頭がイベント。16 通り)・`commercial`(商業誌。10 通り)。差分で新しい名前のプリセットを足せます |
...
**本ごとにプリセットを選べます。** ただし**画面では選びません**(プリセットを分けたいものは、そもそも一緒に読み込まない。
2026-09-21、利用者の判断)。フォルダごとの割り当ては、確かめ用に CLI の `--presets <割り当て.json>` に残してあります:
...
- **欄の多い形を先に、少ない形を後に**書いてください(`… @title` を `… @title (@source)` より先に書くと、末尾の丸括弧まで
  タイトルに入ります)。
- 差分では `"presets": { "commercial": { "formats": { "$add": ["@title - @author"], "at": "end" } } }` のように、プリセットの名前で指します。
  同梱に無い名前を書くと新しいプリセットになります(全体を書く: `{ "my-shelf": { "formats": ["…"] } }`)。
- 書き間違い(知らない予約語など)は、読み込みのときに位置付き(`presets.commercial.formats[0]`)の誤りになります。
...
| `numberWords` | 語 → 語 | 数を表す語(`ふたつ → 2` `みっかめ → 3`)。巻数(表示)は書いてある語のまま |
...
| `sequelWords` | 語 | 本編の後に続くことを示す語(`アフター` `後日談` `その後` `外伝` `おまけ`)。巻数(表示)は名前のとおり、並べる数はシリーズの最後の巻の次 |
...
- `compilations` の `inMainSeries`: 同じ書き手に本編のシリーズ `X` があれば、総集編をそこへ入れます。**本編がシリーズになって
  いなくても、本編の本が残っていれば、その本と総集編で 1 つのシリーズ `X` にします**(`X 11` の 1 冊だけが残った蔵書。
  拾うのは、タイトルが `X` そのものか、そのすぐ後ろが巻だけでできている本に限ります)。本編の本がどこにも無ければ、
  既定と同じく `X 総集編` のシリーズにします。`compilationVolume` の `afterRange` では、収録範囲が読めた総集編の巻を、その最後の巻の直後に
...
| `wordNumber` | `numberWord` | `words` | **数を語で書いた巻**(`ふたつ` = 2、`みっかめ` = 3)。対応表に無い語は読まない。語がひとまとまりで書かれているときだけ(`ふたつの影` は読まない) |
...
| `sequel` | `sequel` | `words` | 本編の後に続く 1 冊(`アフターエピソード` `後日談` `その後` `外伝` `おまけ`)。**シリーズ名より後ろがこの語で始まるとき**、残り全体を巻の表記にし、並べる数はそのシリーズの**最後の番号 + 1**(`後日談 2` のように番号が続けば + その数)。総集編・番外編はオフセットで後ろへ置いた本なので、最後の番号には数えない。番号がどこにも無いシリーズでは、表記だけを入れて数は付けない |
