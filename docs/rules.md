...
| `@author` | `authors` | 作者(`split` の文字で複数に分ける) | CSV の「作者」、Stackroom XML の `Author`(2 つ目以降) |
...
- `engine` は照合の処理(qooLibrary)での呼び名、`field` は qooMeta の欄です。どちらも処理と結び付いているので、差分では変えられません。
- 差分で変えられるのは `@author` の `split`(作者を分ける文字。既定は `、` `,` `，` `&` `＆` `/` `／`)だけです。
...
命名の流儀ごとのフォーマットの組。**上から試し、どれかのフォーマットが一致した最初のプロファイルを採ります。**
今は同人誌向けの `doujinshi` だけです。差分ではプロファイルを ID で指します:
`"profiles": { "doujinshi": { "formats": { "$add": ["[@circle] @title {@keywordA}"], "at": "end" } } }`。

| キー | 意味 |
|---|---|
| `id` | プロファイルの ID |
| `delimiters` | 区切りとして扱う括弧の組(`["開き", "閉じ"]`)。既定は `[ ]` と `( )` だけ。差分では `{ "$replace": [...] }` で丸ごと置き換える |
| `protectedTokens` | 1 かたまりとして扱う文字列(正規表現) |
| `formats` | フォーマット。**上から順に試し、最初に一致したものを採る**(書く順が優先順位)。差分の `$add` は `"at": "start"`(既定。利用者の形を先に試す)か `"end"` |

`delimiters`: `【】` `「」` `（）` はタイトルの一部として使われることが多いので、区切りにしていません。足すと、タイトルの途中の
括弧で誤って区切られることがあります。

`formats` の書き方:

- 予約語以外の文字(括弧・空白・その他の文字)は、そのまま照合します。空白は「0 文字以上の空白」として照合します。
...
- 書き間違い(知らない予約語など)は、読み込みのときに位置付きのエラーになります。

`protectedTokens`: ここに一致した部分は、区切りの括弧を含んでいても区切りとして扱いません。既定は、タイトルの末尾の `(2019)` の
ような年と、`(完結)` などの語です。これが無いと、`月の庭 (2019)` の `(2019)` がネタ(`@relation`)として読まれてしまいます。
正規表現は JSON の文字列なので、`\` は `\\` と書きます。

### `fallback` — どのフォーマットにも一致しないとき

| 規則 | 既定 | 意味 |
|---|---|---|
| `simpleBrackets` | 有効 | 括弧の位置だけで読む(`(A) [B (C)] T (D)` の形を探す)。止めると、名前全体をタイトルにする |
...
**今の版で選べるのは上の 4 つ**(`subtitled`・`differentRelation`・`differentGenre`・`unnumberedFirst`)の既定でない値までです。
下の 5 つの既定でない値は、読み込みのときに「まだ働かない設定」のエラーになります(選んだつもりで効いていない、を防ぐため)。
