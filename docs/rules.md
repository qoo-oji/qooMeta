...
| `sequel` | `sequel` | `words` | **既定では止めてある**(入にすると効く)。本編の後に続く 1 冊(`アフターエピソード` `後日談` `外伝` `番外編` `特別編` `おまけ` …)。英字は大文字小文字を問わない。**シリーズ名より後ろがこの語で始まるとき**、残り全体を巻の表記にし、並べる数はそのシリーズの**最後の番号 + 1**(`後日談 2` のように番号が続けば + その数)。総集編・番外編はオフセットで後ろへ置いた本なので、最後の番号には数えない。番号がどこにも無いシリーズでは、表記だけを入れて数は付けない |
